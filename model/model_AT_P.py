    ...
    def forward(self, U_v, U_a, U_t, U_p, M_v, M_a, M_t, seq_lengths, target_loc, seg_len, n_c):
        ...
        for i in range(M_v.shape[0]):
            ...
            for k in range(n_c[i]):
                new_inp_A, new_inp_T = inp_A.clone(), inp_T.clone()
                
                new_inp_AT = inp_A.clone()
                new_inp_TA = inp_T.clone()
                ...
                att_AT,_ = self.attn(inner_AT_, inner_TA_, inner_TA_, mask_A[:, k])
                
                att_TA,_ = self.attn(inner_TA_, inner_AT_, inner_AT_, mask_T[:, k])
                ...
                # Multimodal fusion
                inner_U = self.fusion_layer(torch.cat([inner_AT, inner_TA, inp_P[0][k]]))
