        ...
        for i in range(M_v.shape[0]):
            ...
            for k in range(n_c[i]):
                ...
                for j in range(seg_len[i]):
                    ...
                    att_V_A, _ = self.attn(new_inp_V[j, :], new_inp_A[j, :], new_inp_A[j, :], mask_V[j, :])
                    att_A_T, _ = self.attn(new_inp_A[j, :], new_inp_T[j, :], new_inp_T[j, :], mask_A[j, :])
                    att_A_V, _ = self.attn(new_inp_A[j, :], new_inp_V[j, :], new_inp_V[j, :], mask_A[j, :])
                    att_T_A, _ = self.attn(new_inp_T[j, :], new_inp_A[j, :], new_inp_A[j, :], mask_T[j, :])
                    att_T_V, _ = self.attn(new_inp_T[j, :], new_inp_V[j, :], new_inp_V[j, :], mask_T[j, :])
                    new_inp_VT[j, :] = att_V_T + new_inp_T[j, :]
                    new_inp_VA[j, :] = att_V_A + new_inp_A[j, :]
                    new_inp_AT[j, :] = att_A_T + new_inp_T[j, :]
                    new_inp_AV[j, :] = att_A_V + new_inp_V[j, :]
                    new_inp_TA[j, :] = att_T_A + new_inp_A[j, :]
                    new_inp_TV[j, :] = att_T_V + new_inp_V[j, :]
                ...
                inner_VT = (att_VT[target_moment] + inner_VA_[target_moment]).squeeze()
                inner_VA = (att_VA[target_moment] + inner_VT_[target_moment]).squeeze()
                inner_AT = (att_AT[target_moment] + inner_AV_[target_moment]).squeeze()
                inner_AV = (att_AV[target_moment] + inner_AT_[target_moment]).squeeze()
                inner_TA = (att_TA[target_moment] + inner_TV_[target_moment]).squeeze()
                inner_TV = (att_TV[target_moment] + inner_TA_[target_moment]).squeeze()
