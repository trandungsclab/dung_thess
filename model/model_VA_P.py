    ...
    def forward(self, U_v, U_a, U_t, U_p, M_v, M_a, M_t, seq_lengths, target_loc, seg_len, n_c):
        ...
        for i in range(M_v.shape[0]):
            ...
            for k in range(n_c[i]):
                new_inp_A,  new_inp_V = inp_A.clone(), inp_V.clone(),
                new_inp_VA = inp_V.clone()
                new_inp_AV = inp_A.clone()
                ...
                att_VA,_ = self.attn(inner_VA_, inner_AV_, inner_AV_, mask_V[:, k])
                att_AV,_ = self.attn(inner_AV_, inner_VA_, inner_VA_, mask_A[:, k])
                ...
                # Multimodal fusion
                inner_U = self.fusion_layer(torch.cat([inner_VA, inner_AV, inp_P[0][k]]))
